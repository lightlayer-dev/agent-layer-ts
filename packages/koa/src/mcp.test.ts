...
function createApp(config: McpServerConfig = testConfig) {
  ...
  const mcpRouter = mcp.router();
  // Mount MCP router under /mcp
  const Router = require("@koa/router").default || require("@koa/router");
  const parentRouter = new Router();
  parentRouter.use("/mcp", mcpRouter.routes(), mcpRouter.allowedMethods());
  app.use(parentRouter.routes());
  app.use(parentRouter.allowedMethods());
  return { app, mcp };
}

describe("mcpServer (Koa)", () => {
...
describe("POST /mcp — JSON-RPC messages (Koa)", () => {
...
describe("GET /mcp — SSE stream (Koa)", () => {
...
describe("DELETE /mcp — session end (Koa)", () => {
  it("returns 200 OK", async () => {
    const { app } = createApp();
    const res = await request(app.callback()).delete("/mcp");
    expect(res.status).toBe(200);
